...
def add_review(product_id, rating, comment, current_user, db):
    ...
    if not has_bought:
        raise HTTPException(403, "Only people who bought this product can add a review")
    ...
    review = Review(
        product_id=product_id, user_id=current_user.id, rating=rating, comment=comment
    )

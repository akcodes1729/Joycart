...
from app.reviews.services.reviews_service import (
    add_review,
    load_reviews,
    rating_calculation,
)
...
def load_reviews_endpoint(product_id: int, db: Session = Depends(get_db)):
    return load_reviews(product_id, db)


@router.get("/reviews/calculate")
def calculate_rating(product_id: int, db: Session = Depends(get_db)):

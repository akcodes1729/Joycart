...
def home(current_user, db):
    products = get_all_products_cached(
        db
    )  # use (products = list_products(db)) in case of db
    ...
    if current_user.is_seller:
        ...
        if seller:
            products = [
                ...
                for p in products
                if p["seller_id"]
                != seller.id  # use (if p.seller_id != seller.id) in case of db .
            ]
    return products
...
def profile(section, edit, current_user, db):
    ...
    if section == "address":
        ...
        if edit:
            address_to_edit = (
                db.query(Address)
                .filter(Address.id == edit, Address.user_id == current_user.id)
                .first()
            )
...
def delete_address(address_id, current_user, db):
    ...
    if address_count <= 1:
        raise HTTPException(
            status_code=400, detail="You must have at least one address"
        )

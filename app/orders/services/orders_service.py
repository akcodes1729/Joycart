...
def single_order_item(item_id, current_user, db):
    ...
    row = (
        ...
        .join(Product, Product.id == OrderItems.product_id)
        .filter(OrderItems.id == item_id, Order.user_id == current_user.id)
        .first()
    )
...
def initiate_razorpay_refund(refund, db):
    ...
    try:
        amount_paise = int(
            (refund.amount * Decimal("100")).quantize(
        ...
        razorpay_refund = razorpay_client.payment.refund(
            ...
            {"amount": amount_paise, "notes": {"order_item_id": refund.orderitem_id}},
        )
